...
### 1️⃣ 데이터 호출 위치 상향 (재호출 방지)

적금 상품 데이터는 `SavingsCalculatorPage`에서 `useGetProductList()`를 통해  
**상위 컴포넌트에서 한 번만 호출**합니다.

이 설계는 다음 전제를 기반으로 합니다.

- 모든 상품 데이터를 **초기 1회 수신**합니다.
- 월 납입액 / 저축 기간 변경은 **클라이언트에서 필터링으로 처리**합니다.
...
과정에서도 **불필요한 API 재호출이 발생하지 않습니다.**

> ⚠️ 만약 필터링 조건에 따라 서버에 매번 재요청해야 하는 구조라면  
> 해당 설계는 의미가 없어지며,  
> 그 경우에는 **쿼리 파라미터 기반 API 호출 구조로 재설계가 필요합니다.**

### 2️⃣ 탭 콘텐츠 분리로 관심사 분리

`TabContents` 컴포넌트를 통해 탭별 렌더링 책임을 분리했습니다.

- `products` 탭 → `SavingsProducts`
- `results` 탭 → `SavingsResults`

이를 통해

- Page 컴포넌트는 **입력 UI 및 상태 관리**
- 하위 컴포넌트는 **목록 렌더링 / 계산 결과 표현**

으로 역할이 명확히 분리되며,  
향후 탭 추가나 계산 로직 변경 시 영향 범위를 최소화할 수 있습니다.
...
- 입력 중 결과 화면이 즉시 변경되어 맥락을 잃지 않도록 다음 전략을 검토 중입니다.
  ...
  - 입력 확정 시 계산
  - `useDeferredValue` 적용  
    (입력값은 즉시 유지하되, 결과 영역은 지연된 값 기준으로 업데이트합니다.)
...
- 개선 방향
  ...
  - 이를 통해 사용자가
    ...
    - 입력값 조정 방향에 대한 힌트를 얻을 수 있도록 설계했습니다.
